...
from src.parser.evtc_handler import EVTCHandler
...
class GW2LogParser(BaseParser):
    ...
    def parse_file(self, file_path: str) -> Dict[str, Any]:
        ...
        if ext in ['.json']:
            ...
            raise ValueError(f"Unsupported file format: {ext}. Only JSON and ZEVTC files are supported.")
    ...
    def _process_json_data(self, data: Dict[str, Any], file_path: str) -> Dict[str, Any]:
        """
        处理JSON数据，标准化为统一格式
        ...
        players = PlayerParser.parse_all(
            data.get('players', []),
            duration_s
        )
