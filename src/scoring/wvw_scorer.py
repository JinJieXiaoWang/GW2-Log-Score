...
class WvWScorer(BaseScorer):
    ...
    def _calculate_player_score(
        ...
        if role == 'DPS':
            ...
            return self._calculate_support_score(player, averages, duration_min, is_small_squad)
        else:
            return self._calculate_utility_score(player, averages, duration_min, is_small_squad)
    ...
    def _calculate_dps_score(
        ...
        if is_small_squad:
            ...
            weights = self.config['WvW']['DPS']['weights']
    ...
    def _calculate_support_score(
        ...
        Returns:
            评分结果
        """
        weights = self.config['WvW']['SUPPORT']['weights']
        ...
        s_cleanse = self.clamp(self.safe_divide(cleanses_per_min * 100, averages['avg_cleanses_per_min']))
        s_strips = self.clamp(self.safe_divide(strips_per_min * 100, averages['avg_strips_per_min']))
        s_survival = SurvivalCalculator.calculate_from_player(player)
    ...
    def _calculate_utility_score(
        ...
        Returns:
            评分结果
        """
        weights = self.config['WvW']['UTILITY']['weights']
