...
class BaseScorer(ABC):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        Args:
            config: 可选的评分配置
        """
        self.config = config or config_loader.scoring_rules_config
...
class MetricsCalculator:
    ...
    def calculate_all_averages(
        cls, players: List[Dict[str, Any]], duration_seconds: float
    ) -> Dict[str, float]:
        """
        计算所有常用团队平均值
        Args:
        ...
        return {
            ...
            "avg_cleanses_per_min": max(
                1,
                cls.calculate_team_average(
                    [
                        {"cleanses_per_min": p.get("cleanses", 0) / duration_min}
                        for p in players
                    ],
                    "cleanses_per_min",
                ),
            ),
            "avg_strips_per_min": max(
                1,
                cls.calculate_team_average(
                    [
                        {"strips_per_min": p.get("strips", 0) / duration_min}
                        for p in players
                    ],
                    "strips_per_min",
                ),
