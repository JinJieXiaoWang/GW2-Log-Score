...
class PVEScorer(BaseScorer):
    ...
    def calculate(self, parsed_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # 如果没有 PVE 配置，返回简单的评分结果
        if "PVE" not in self.config:
            scores = []
                ...
                scores.append(
                    {
                        ...
                        "account": player.get("account", "Unknown"),
        ...
        averages = MetricsCalculator.calculate_all_averages(
            players, parsed_data.get("duration", 1)
        )
    ...
    def _calculate_player_score(
        self, player: Dict[str, Any], averages: Dict[str, float]
    ) -> Dict[str, Any]:
        """
        计算单个玩家的PVE评分
