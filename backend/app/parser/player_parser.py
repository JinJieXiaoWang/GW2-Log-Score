...
class PlayerParser:
    ...
    @classmethod
    def parse(
        cls, player_data: Dict[str, Any], duration_seconds: float
    ) -> Dict[str, Any]:
        """
        解析单个玩家数据
        ...
        """
        dps_all = (
            player_data.get("dpsAll", [{}])[0] if player_data.get("dpsAll") else {}
        )
        stats_all = (
            player_data.get("statsAll", [{}])[0] if player_data.get("statsAll") else {}
        )
        defenses = (
            player_data.get("defenses", [{}])[0] if player_data.get("defenses") else {}
        )
        support = (
            player_data.get("support", [{}])[0] if player_data.get("support") else {}
        )
    ...
    @classmethod
    def parse_all(
        cls, players_data: List[Dict[str, Any]], duration_seconds: float
    ) -> List[Dict[str, Any]]:
        """
        解析所有玩家数据
        Args:
