...
class WvWRuleScorer(BaseScorer):
    ...
    def _calculate_player_score(
        ...
        Args:
            player: 玩家数据
            averages: 团队平均�?            duration_min: 战斗时长(分钟)
            is_small_squad: 是否为毒瘤小�?
        Returns:
            评分结果
        """
    ...
    def _extract_value_from_player(
        ...
        Args:
            player: 玩家数据
            data_source: 数据源描�?            duration_min: 战斗时长(分钟)
            averages: 团队平均�?
        Returns:
            提取的�?        """
        if not data_source:
        ...
        # 处理计算表达�?        if "deaths" in data_source and "downs" in data_source:
            deaths = player.get("deaths", 0)
            downs = player.get("downs", 0)
            return max(0, 100 - (deaths * 20 + downs * 10))

        # 处理组合数据�?        if "+" in data_source:
    ...
    def _extract_single_value(
        ...
        # Buff 覆盖�?        buff_match = re.match(r'buff_uptimes\[(\w+)\]', field, re.IGNORECASE) or re.match(r'buffUptimes\[(\w+)\]', field)
        if buff_match:
            buff_name = buff_match.group(1)
            return self._get_buff_uptime(player, buff_name)

        # Debuff 覆盖�?        debuff_match = re.match(r'debuff_uptimes\[(\w+)\]', field, re.IGNORECASE) or re.match(r'debuffUptimes\[(\w+)\]', field)
        if debuff_match:
            debuff_name = debuff_match.group(1)
            return self._get_debuff_uptime(player, debuff_name)

        # 每分钟统�?        if "_per_min" in field.lower() or "_per_minute" in field.lower():
