"""
WvW EVTC / ZEVTC 战斗日志解析�? v2.0
======================================
功能�?  - 解析 arcdps 原始 .evtc / .zevtc 二进制日志（支持 revision 0/1�?  - 提取 WvW 大团/毒瘤 核心评分所需字段
  - 职业 ID �?名称映射（profession + elite spec�?  - 玩法类型自动判断（大�?/ 毒瘤�?  - WvW 大团 / 毒瘤 评分计算（权重可配置�?  - SQLite 持久化（玩家表、战斗表、评分表、出勤表�?  - JSON 输出（校�?对比用）
  - CSV 报表导出
  - 批量处理整个文件�?
用法�?  # 单文�?  python wvw_evtc_parser.py log.zevtc

  # 批量
  python wvw_evtc_parser.py --batch /path/to/logs/

  # 查询历史
  python wvw_evtc_parser.py --query --player 帅姐�?
  # 导出报表
...
# elite spec id -> 专精名（经验证的正确映射，基�?EI JSON �?zevtc 原始数据�?# 验证日期: 2026-04-08
# 来源: tests/build_elite_mapping.py
#
# 注意：同一�?elite ID 可能对应多个专精（取决于 GW2 版本或服务器�?# 下面�?ID 分组列出所有可能的映射
#
# 经验证的映射（来�?20260408-222901.zevtc 测试文件�?
...
#
# EI JSON 解析�?Unknown �?elite ID:
#   0x49, 0x4A, 0x4B, 0x4E, 0x50, 0x51 (需要更多数据验�?
...
# 未验证但可能正确的映射（来自 GW2 API �?arcdps 文档�?ELITE_SPEC_MAP_UNVERIFIED = {
    0x04: "Berserker",
    0x06: "Scrapper",
    0x08: "Daredevil",
    0x09: "Tempest",
...
# 最终映射表 = 未验证映�?+ 验证映射（验证的值会覆盖冲突的未验证值）
ELITE_SPEC_MAP = {**ELITE_SPEC_MAP_UNVERIFIED, **ELITE_SPEC_MAP_VERIFIED}

# WvW 常用 BUFF ID（来�?arcdps EVTC 规范 + GW2 API�?BUFF_IDS = {
    1122: "stability",
    26980: "resistance",
    717: "protection",
    719: "swiftness",
...
class PlayerStats:
    ...
    # 对敌效果
    downs_inflicted: int = 0  # 击倒敌�?    kills_inflicted: int = 0  # 击杀敌方（击败）

    # 辅助
    boon_strips: int = 0  # 撕BUFF
    condi_cleanses: int = 0  # 清除己方条件

    # BUFF 覆盖率（秒数，除�?duration 得到 %�?    buff_uptime: Dict[str, float] = field(default_factory=dict)

    @property
    def survival_score_raw(self) -> float:
        """存活能力原始分（倒地/死亡越少越高，满�?.0�?""
...
class ZevtcParser:
    ...
    def _load(self):
        if zipfile.is_zipfile(self.path):
            ...
            with open(self.path, "rb") as f:
                if os.path.getsize(self.path) > 10 * 1024 * 1024:  # 10MB以上使用mmap
    ...
    def parse(self) -> Tuple[CombatMeta, List[AgentInfo], List[PlayerStats]]:
        ...
        # 遍历一：建�?instid→addr 映射 + 提取元数�?+ 团队ID
        ...
        for i in range(n_events):
            ...
            # instid 映射（从进战/复活/生成事件�?            if sc in (
                SC_ENTER_COMBAT,
                SC_CHANGE_UP,
                SC_SPAWN,
                SC_EXIT_COMBAT,
        ...
        for ag in agents_list:
            # 玩家筛选条件：
            # 1. 必须是玩家（elite不是0xFFFFFFFF�?            # 2. prof必须�?-9范围内（有效职业ID�?            if ag.is_player and 0 <= ag.prof <= 9:
                # 排除重复地址的agent
...
def _calc_buff_uptime(events: list, duration_ms: int) -> int:
    """
    �?apply/remove 事件序列计算 buff �?[0, duration_ms] 内的总覆盖时间（ms�?
    Args:
    ...
    for s, e in intervals[1:]:
        if s <= merged[-1][1]:
            ...
            merged.append((s, e))
    # 截到战斗时间�?    total = sum(e - s for s, e in merged)
...
# ══════════════════════════════════════════════════════════════�?# 评分模块（权重可配置�?# ══════════════════════════════════════════════════════════════�?
# 默认权重（可通过外部 JSON 覆盖�?DEFAULT_WEIGHTS = {
    "zerg": {
        "dps": {
            "effective_damage": 40,  # 总伤害（WvW 有效伤害，含 NPC�?            "breakbar_damage": 30,  # 破控
            "survival": 30,  # 存活（倒地/死亡惩罚�?        },
        "support": {
            "stability_uptime": 20,  # 稳固覆盖�?            "resistance_uptime": 15,  # 抗性覆盖率
            "boon_strips": 20,  # �?BUFF
            "condi_cleanses": 15,  # 清条�?            "survival": 30,  # 存活
        },
    },
    "havoc": {
        "kills": 40,
...
def _normalize_score(raw: float, team_values: list, max_pts: float) -> float:
    """
    相对评分：以团队最高值为 100%，该玩家按比例得分�?    如果团队最高值为 0，所有人�?max_pts�?    """
...
def save_to_db(
    ...
    # 插入 players（upsert�?    for p in players:
        conn.execute(
            """
    ...
    conn.commit()
    print(
        f"[DB]  已存�? {os.path.basename(file_path)}  log_id={log_id}  {play_style}  {fight_date}"
    )
...
def export_csv(
    ...
    with open(out_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in rows:
            ...
            w.writerow(row)
    print(f"[CSV] 已导�?{len(rows)} 条记�?�?{out_path}")
...
def query_history(
    ...
    print(
        f"\n{'日期':<12} {'地图':<24} {'玩法':<8} {'名称':<16} {'职业':<14} {'角色':<8} {'总分':>6} {'伤害':>10} {'�?:>4} {'�?:>4}"
    )
    print("─" * 110)
    for r in rows:
        print(
...
def attendance_report(conn: sqlite3.Connection, period: str = "month"):
    ...
    print(
        f"\n{'账号':<24} {'名称':<14} {'职业':<14} {'出勤天数':>8} {'出勤场次':>8} {'出勤�?:>8}"
    )
    print("─" * 80)
    for r in rows:
...
def process_file(
    ...
    if verbose:
        ...
        print(f"  玩法   : {play_style}  玩家�? {len(players)}")
        print(f"{'─'*60}")
        print(
            f"  {'名称':<16} {'职业':<14} {'角色':<8} {'总分':>6} {'总伤�?:>10} {'倒地':>4} {'死亡':>4} {'破控':>8}"
        )
        for s in scores:
            r = s["raw"]
            print(
...
def main():
    ap = argparse.ArgumentParser(description="WvW arcdps EVTC/ZEVTC 解析�?v2.0")
    ap.add_argument(
        "input", nargs="?", help=".evtc/.zevtc 文件或文件夹（批量时�?--batch�?
    )
    ap.add_argument(
        "--batch", action="store_true", help="批量解析文件夹内所�?.evtc/.zevtc"
    )
    ap.add_argument("--db", default="wvw_logs.db", help="SQLite 数据库路�?)
    ...
    ap.add_argument("--days", type=int, default=30, help="查询最近N�?)
    ap.add_argument("--style", choices=["zerg", "havoc"], help="玩法类型过滤")
    ap.add_argument("--period", default="month", choices=["week", "month", "year"])
    args = ap.parse_args()

    # 查询/报表模式（不需�?input�?    if args.query or args.attendance or args.export:
        conn = get_db(args.db)
        if args.query:
            query_history(
                conn,
