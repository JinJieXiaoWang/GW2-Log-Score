#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
验证字典分组命名标准化
"""

import os
import sys
import sqlite3

# 添加项目根目录到 Python 路径
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from app.database.db_manager import DBManager
from app.database.dict_init import DictionaryDataInitializer

def verify_dict_naming():
    """
    验证字典分组命名是否符合 gw2_ 前缀规范
    """
    print("=== 验证字典分组命名标准化 ===")
    
    # 创建数据库管理器
    db_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "database",
        "gw2_logs.db"
    )
    
    print("数据库路径:", db_path)
    
    try:
        # 初始化数据库管理器
        db = DBManager(db_path)
        print("Database manager initialized successfully")
        
        # 检查字典类型
        with sqlite3.connect(db.db_path) as conn:
            cursor = conn.cursor()
            
            # 获取所有字典类型
            cursor.execute("SELECT dict_type, dict_name FROM sys_dict_type ORDER BY sort_order")
            dict_types = cursor.fetchall()
            
            print("\nCurrent dict types:")
            print("-" * 60)
            
            all_valid = True
            for dict_type, dict_name in dict_types:
                is_valid = dict_type.startswith('gw2_')
                status = "OK" if is_valid else "ERROR"
                print(f"{status} {dict_type:<40} {dict_name}")
                if not is_valid:
                    all_valid = False
            
            print("-" * 60)
            
            # 统计结果
            total_types = len(dict_types)
            valid_types = sum(1 for dt, _ in dict_types if dt.startswith('gw2_'))
            
            print("\nStatistics:")
            print(f"Total dict types: {total_types}")
            print(f"Valid types: {valid_types}")
            print(f"Invalid types: {total_types - valid_types}")
            
            if all_valid:
                print("\nAll dict group names follow the 'gw2_' prefix convention")
            else:
                print("\nSome dict group names do not follow the 'gw2_' prefix convention")
            
            # 检查字典数据
            print("\nChecking dict data:")
            for dict_type, dict_name in dict_types:
                cursor.execute("SELECT COUNT(*) FROM sys_dict_data WHERE dict_type = ?", (dict_type,))
                count = cursor.fetchone()[0]
                print(f"{dict_type}: {count} records")
        
        print("\n=== Verification completed ===")
        return all_valid
        
    except Exception as e:
        print("Verification failed:", e)
        import traceback
        traceback.print_exc()
        return False

def reinitialize_dict():
    """
    重新初始化字典数据
    """
    print("\n=== Reinitializing dict data ===")
    
    db_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        "database",
        "gw2_logs.db"
    )
    
    try:
        # 初始化数据库管理器
        db = DBManager(db_path)
        
        # 创建字典初始化器
        initializer = DictionaryDataInitializer(db)
        
        # 执行初始化
        results = initializer.initialize_all()
        
        print("\nInitialization results:")
        for dict_type, result in results.items():
            print(f"{dict_type}: created {result['created']}, skipped {result['skipped']}")
        
        print("\nDict data reinitialized successfully")
        return True
        
    except Exception as e:
        print("Initialization failed:", e)
        import traceback
        traceback.print_exc()
        return False

if __name__ == "__main__":
    # 强制重新初始化
    print("Forcing dict data initialization...")
    reinitialize_dict()
    
    # 验证结果
    print("\nVerifying after initialization:")
    verify_dict_naming()
